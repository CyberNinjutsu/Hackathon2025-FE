...
export const useAdminAuth = () => {
  ...
  const initializeAuth = useCallback(() => {
    try {
      ...
      setState(prev => ({
        ...
        step: 'email',
        isLoading: false,
        lockoutUntil: isLocked ? new Date(Date.now() + lockoutTime * 1000) : null
      }));
   ...
   */
  const submitEmail = useCallback(async (email: string): Promise<LoginResult> => {
    try {
      ...
      if (!rateLimiter.canRequestOTP(email)) {
        ...
        const result: LoginResult = {
          success: false,
          error: {
            type: isLocked ? AdminAuthError.ACCOUNT_LOCKED : AdminAuthError.RATE_LIMITED,
            message: isLocked 
              ? `Account is locked. Try again in ${Math.ceil(waitTime / 60)} minutes.`
        ...
        setState(prev => ({
          ...
          lockoutUntil: isLocked ? new Date(Date.now() + waitTime * 1000) : null
        }));
      ...
      const expiresAt = otpService.storeOTP(email, otp);
      ...
      const expiryTime = Math.floor((expiresAt.getTime() - now.getTime()) / 1000);
   ...
   * Verify OTP code
   */
  const verifyOTP = useCallback(async (otp: string): Promise<VerificationResult> => {
    try {
      setState(prev => ({ ...prev, isLoading: true, error: null }));
      ...
      // Validate OTP
      const validation = otpService.validateOTP(state.email, otp);
      ...
      if (!validation.isValid) {
        ...
        if (isNowLocked) {
          ...
          setState(prev => ({
            ...prev,
            isLoading: false,
            error: `Too many failed attempts. Account locked for ${Math.ceil(lockoutTime / 60)} minutes.`,
